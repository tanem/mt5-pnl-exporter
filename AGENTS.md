...
- `snapshot.py` — typed pydantic models for `AccountSnapshot`, `ClosedDeal`, `OpenPosition`, `CashFlow`, `Order`, `SymbolInfo` + atomic `write` (temp file + `replace`). `write` and `read` chain `JSON → gzip → age (passphrase)` on disk; both take the passphrase as a required argument. `read()` accepts same-major snapshots up to its own minor (currently `"1.1"`); rejects others with a readable error. `Snapshot` also carries `entry_deals` (`ClosedDeal`-shaped opening deals), `orders` (`Order`, all states), and `symbols` (`SymbolInfo`) — all three default to an empty list, so a `1.0` snapshot still reads. One record per closed deal, position, and cash flow — no pre-aggregation.
...
- **Deal classification**: `MT5Source.fetch_closed_deals` keeps only `DEAL_ENTRY_OUT`/`INOUT`/`OUT_BY` records with non-balance-family types (all three carry realised P&L; `INOUT` is a reversal, `OUT_BY` a close-by). `fetch_entry_deals` keeps only `DEAL_ENTRY_IN` records with non-balance-family types (the opening half of a position). `fetch_cash_flows` keeps only balance-family types (`BALANCE`, `CREDIT`, `CHARGE`, `CORRECTION`, `BONUS`, `COMMISSION`). `_get_history_raw` memoises `history_deals_get` per `(login, date_from, date_to)` so `fetch_closed_deals` and `fetch_entry_deals` share one round-trip to MT5. `fetch_orders` returns every order in the window regardless of state (filled, cancelled, rejected, …) via `history_orders_get`, memoised per `(login, date_from, date_to)` in `_orders_cache` (cleared in `shutdown`). `fetch_symbols` collects the distinct symbols seen across deals and orders and calls `symbol_info` once per name.
...
- **`SCHEMA_VERSION` is `"1.1"`** (major.minor string). `read()` accepts `"1.0"` and `"1.1"` — the same major up to its own minor; bump the minor for additive fields, the major for breaking changes.
...
Public, open-source project with a global audience. In committed artifacts
(specs, plans, READMEs, code, comments, commit messages):

- No contributor-specific detail: personal financial or tax specifics, broker
  names, account numbers, or local file paths.
- Keep tax, jurisdiction and currency examples generic. If a specific regime is
  named, label it as one example among many rather than the default.
